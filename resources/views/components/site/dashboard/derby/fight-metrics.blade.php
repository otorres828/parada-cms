...
|--------------------------------------------------------------------------
--}}
@props(['peleaActiva', 'modalidades', 'volumenVerde' => 0, 'volumenRojo' => 0, 'usuariosEnVivo' => 0])
                    ...
                    <div class="bg-surface-container-lowest/40 border border-white/5 rounded-xl p-2.5 flex flex-col items-center @if($mod->id != 1) mb-2 @endif">
                        ...
                        <div class="p-1.5 rounded-lg ">
                            ...
                            <span class="text-white font-mono font-black text-xs">
                                ${{ number_format($peleaActiva ? $peleaActiva->getVolumenPorModalidad(1, 1, $mod->id) : 0, 2) }}
                            </span>
                        </div>
                        ...
                        <div class="bg-surface-container-lowest/40 border border-white/5 rounded-xl p-2.5 flex flex-col items-center">
                            <div class="p-1.5 rounded-lg">
                                <span class="text-red-400 block text-md font-bold  uppercase">Agarro
                                    {{ $mod->nombre }}</span>
                                <span class="text-white font-mono font-black text-xs">
                                    ${{ number_format($peleaActiva ? $peleaActiva->getVolumenPorModalidad(1, 2, $mod->id) : 0, 2) }}
                                </span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div
                        ...
                        <div class=" p-1.5 rounded-lg">
                            <span
                                ...
                                {{ $mod->nombre }}</span>
                            <span class="text-white font-mono font-black text-xs">
                                ${{ number_format($peleaActiva ? $peleaActiva->getVolumenPorModalidad(2, 2, $mod->id) : 0, 2) }}
                            </span>
                        </div>
                        ...
                        <div
                            ...
                            <div class=" p-1.5 rounded-lg">
                                <span class="text-green-400 block text-md font-bold  uppercase">Doy
                                    {{ $mod->nombre }}</span>
                                <span class="text-white font-mono font-black text-xs">
                                    ${{ number_format($peleaActiva ? $peleaActiva->getVolumenPorModalidad(2, 1, $mod->id) : 0, 2) }}
                                </span>
                            </div>
...
<div class="border-t border-white/5 pt-4 space-y-3">
    <div class="flex justify-between items-center text-xs">
        <span class="text-red-400 font-medium flex items-center gap-1.5">
            <span class="material-symbols-outlined text-sm text-red-400">payments</span>
            Total Rojo:
        </span>
        <span class="text-white font-mono font-bold">${{ number_format($volumenRojo, 2) }}</span>
    </div>

    <div class="flex justify-between items-center text-xs">
        <span class="text-green-400 font-medium flex items-center gap-1.5">
            <span class="material-symbols-outlined text-sm text-green-400">payments</span>
            Total Verde:
        </span>
        <span class="text-white font-mono font-bold">${{ number_format($volumenVerde, 2) }}</span>
    </div>
