...
|    - <x-site.dashboard.bet-selector -> Selector de modalidades y cuotas de apuesta, con reactividad local.
|    - <x-site.dashboard.list-fight-in-live /> -> Listado de apuestas activas del usuario, con cancelación y estado.
|--------------------------------------------------------------------------
--}}
@props(['apuestaAbierta', 'peleaActiva', 'modalidades', 'balanceUsuario'])
...
<div x-data="bettingSlip({{ (float) $balanceUsuario }}, {{ json_encode($modalidades->pluck('nombre', 'id')->toArray() ?? []) }})"
    ...
    <div class="flex items-center justify-between mb-6">
        <h4 class="font-headline-md text-base md:text-headline-md text-on-surface-variant">
            {{ $apuestaAbierta ? 'Apuestas Abiertas' : 'Apuestas Cerradas' }}
        </h4>

        <span
            class="bg-surface-container border border-white/10 text-primary font-black text-[10px] px-2.5 py-1 rounded-md uppercase tracking-wider">
            {{ $peleaActiva ? "Pelea #{$peleaActiva->id}" : 'Cerrado' }}
        </span>
    </div>
    ...
    @php
        $misApuestasActivas = $this->apuestasUsuarioLista ?? collect();
    @endphp

    <x-site.dashboard.list-fight-in-live :misApuestasActivas="$misApuestasActivas" />
    ...
    <script>
        Alpine.data('bettingSlip', (balanceInicial, modalidadesMap) => ({
            ...
            seleccionarCuota(gallo, modalidad, id, porcentaje, bando) {
                ...
                console.log(`Seleccionado: Gallo ${gallo}, Modalidad ${modalidad}, ID ${id}, Porcentaje ${porcentaje}, Bando ${bando}`);
            },
            ...
            confirmarApuesta() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        this.$wire.procesarApuestaDesdeSlip(
                            this.betAmount,
                            this.tipoGallo,
            ...
            cancelarApuesta(apuestaId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        this.$wire.cancelarApuesta(apuestaId);
                    }
                });
            }
        }));
